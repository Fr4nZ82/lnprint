//plugin modified with crop and resize functionality
(function($) {
	...
	var DisplayPreviewDialog = function(preview, endelem, inforow, data, settings, UpdateCroppedFile) {
		var newPreviewUrl = ($(endelem).css('background-image')).slice(5,-2)
		preview = $('<img>').attr('src', newPreviewUrl).attr('id','imgPreview');
		...
		if (settings.showpreview)  settings.showpreview.call(inforow, data, preview, previewclone,HidePreviewDialog,endelem,UpdateCroppedFile);
	};
	...
	$.fn.FancyFileUpload = function(options,cb) {
		this.each(function() {
			var $this = $(this);
		...
		$(document).off('drop.fancy_fileupload dragover.fancy_fileupload');
		$(document).on('drop.fancy_fileupload dragover.fancy_fileupload', function (e) {e.preventDefault();});
...
`
			...
			var UpdateCroppedFile = function(fileBlob,cb) {

				inforow.remove();

				delete data.ff_info;
				console.log('fileuplad ad on update cropped file, fileblob = ', fileBlob)
				$('#fileupladinput').fileupload('add', { files: [fileBlob] })
			...
			if (URL && URL.createObjectURL){
				...
				if (url)
				{
					...
					{
						var previewButton = inforow.find('.ff_fileupload_preview_image');
						previewButton.css('background-image', 'url("' + url + '")')
						if(data.files[0].mainPhoto == 'yes'){
							previewButton.attr('main','yes')
						}else{
							previewButton.attr('main','no')
						}
						haspreview = true;
						preview = $('<img>').attr('src', url).attr('id','imgPreview');
						hasimage = true;
					}
			...
			if (haspreview){
				inforow.find('.ff_fileupload_preview_image').addClass('ff_fileupload_preview_image_has_preview').attr('aria-label', Translate('Preview')).click(function(e) {
					...
					this.blur();
					DisplayPreviewDialog(preview, this, inforow, data, settings, UpdateCroppedFile);
				});
			}
			else{
			...
			if (settings.edit && !data.ff_info.errors.length){
				inforow.find('.ff_fileupload_filename').append($('<input>').attr('type', 'text').attr('cfn',fileext).val(filename))
			}
			else{
			...
			inforow.find('.ff_fileupload_fileinfo').text(data.ff_info.displayfilesize);
			...
			if (!data.ff_info.errors.length){
				inforow.find('.ff_fileupload_actions').append($('<button>').addClass('ff_fileupload_start_upload').attr('type', 'button').attr('aria-label', Translate('Start uploading')).attr('style','display:none;').click(StartUpload));
				inforow.find('.ff_fileupload_actions_mobile').append($('<button>').addClass('ff_fileupload_start_upload').attr('type', 'button').attr('aria-label', Translate('Start uploading')).attr('style','display:none;').click(StartUpload));
		...
		return this.each(function() {
			...
			var uploads = $('<table>').addClass('ff_fileupload_uploads').attr('id', 'ff_filestable');
			...
			var immutableoptions = {
				...
				dropZone: dropzone,
				add: 	function(e, data) {
					console.log('start add',data,e,data.files[0].name.toString())
					if(e.delegatedEvent && e.delegatedEvent.target){
						console.log('add and resize')
						const width = settings.imgWidth;
						...
						reader.onload = event => {
							...
							img.onload = () => {
								...
								ctx.canvas.toBlob(function(blob){
									var mainP
									if(data.files[0].mainPhoto && data.files[0].mainPhoto == 'yes'){
										mainP = 'yes'
									}else{
										mainP = 'no'
									}
									data.files[0] = new File([blob], fileName, {
										type: 'image/jpeg',
										lastModified: Date.now()
									});
									if(mainP == 'yes'){
										data.files[0].mainPhoto = 'yes'
									}
									console.log('before add data.files -> ',data.files[0].name.toString())
